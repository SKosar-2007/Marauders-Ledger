...
VECTORAI_LICENSE = os.getenv("VECTORAI_LICENSE_KEY", "H894W-WBGRK-J74P-HCP8D-JJ9BC-G9V7")
...
def _vt_get_transactions_by_batch(batch_id: str) -> list[dict]:
    ...
    next_offset = None
    while True:
        page = _client.points.scroll("transactions", limit=100, offset=next_offset)
...
def _vt_get_anomalies(user_id: Optional[int] = None, severity: Optional[str] = None) -> list[dict]:
    ...
    next_offset = None
    while True:
        page = _client.points.scroll("anomalies", limit=100, offset=next_offset)
        points = page[0] if isinstance(page, tuple) else page
        for p in points:
            if not p.payload.get("is_anomaly", False):
                continue
            if user_id is not None and p.payload.get("user_id") != user_id:
                continue
...
def _vt_update_anomaly_status(anomaly_id: int, status: str) -> bool:
    ...
    from actian_vectorai import PointStruct
    _client.points.upsert("anomalies", [PointStruct(id=p.id, vector=p.vector, payload=p.payload)])
    return True
...
def _vt_insert_narrative(anomaly_id: int, text: str) -> int:
    from actian_vectorai import PointStruct
    existing = _client.points.scroll("narratives", limit=200)
...
def _vt_get_narrative_by_anomaly_id(anomaly_id: int) -> Optional[dict]:
    next_offset = None
    while True:
        page = _client.points.scroll("narratives", limit=100, offset=next_offset)
        points = page[0] if isinstance(page, tuple) else page
        for p in points:
            if p.payload.get("anomaly_id") == anomaly_id:
...
def _vt_update_narrative_audio(narrative_id: int, audio_data: bytes):
    points = _client.points.get("narratives", ids=[narrative_id])
    if points:
        p = points[0]
        p.payload["audio_data"] = audio_data.hex()
        from actian_vectorai import PointStruct
        _client.points.upsert("narratives", [PointStruct(id=p.id, vector=p.vector, payload=p.payload)])
